...
//
//     if (username != null) {
//         return ResponseEntity.ok(service.listPoliciesByUsername(username));
//     }
//
//     return ResponseEntity.ok(service.listAllPolicies());
// }
...
//         @RequestHeader(value = "X-USERNAME", required = false) String raisedBy,
...
public class SupportController {
    ...
    public ResponseEntity<Ticket> raiseIssue(
        ...
        return ResponseEntity.ok(
                service.raiseIssue(raisedBy, policyId, subject, description)
        );
    }
    ...
    public ResponseEntity<ApiResponse<Ticket>> requestChange(
        ...
        Ticket ticket = service.requestPolicyChange(raisedBy, req);

        return ResponseEntity.ok(
                new ApiResponse<>("Policy change request submitted", ticket)
        );
    }
    ...
    public ResponseEntity<List<Ticket>> listTickets(
        ...
        if (raisedBy != null)
            return ResponseEntity.ok(service.ticketsByUser(raisedBy));

        if (policyId != null)
            return ResponseEntity.ok(service.ticketsByPolicy(policyId));
